...
function renderList(title, rows, emptyText, labelFields, getOnClick) {
  if (!rows.length) {
    return `
      <section>
...
function renderHexScreen(hexId) {
  ...
  document.getElementById('panel-content').innerHTML = `
    ...
    ${renderList(
      ...
      row => `pushPanelScreen(() => renderPoiScreen('${escapeJsString(row.POI_ID)}'))`
    )}
  `;
}
...
function renderPoiScreen(poiId) {
  ...
  document.getElementById('panel-content').innerHTML = `
    <button
      class="panel-link mobile-nav-back"
      onclick="goBackPanel()"
    >
      ← Back
    </button>
      ...
      poi.POI_Type === "Settlement"
        ? `
          <p>
            ...
            ${escapeHtml(poi.Population || "Unknown")}
          </p>
        `
        : ""
    }
    ...
    ${renderList(
      ...
      row => `pushPanelScreen(() => renderNpcScreen('${escapeJsString(row.NPC_ID)}'))`
    )}
  `;
}
...
document.getElementById('mobile-panel-back')
  .addEventListener('click', function () {
    goBackPanel();
  });
