...
function updatePanBounds() {
  ...
  const rightPadding = isMobile ? padding : padding * 1.35;
...
function getNpcsForHex(hexId) {
  ...
  return pois.flatMap(poi => getNpcsForPoi(poi.POI_ID));
}
...
function renderHexPreview(hexId) {
  ...
  document.getElementById("panel-content").innerHTML = `
    ...
    ${countLine ? `
      <p><strong>Known Records:</strong> ${escapeHtml(countLine)}</p>
    ` : ""}

    <h3>Field Notes</h3>
    <p>${renderMultilineText(journalPreview)}</p>

    <button class="codex-link" type="button" onclick="openCodexPage('hex', '${escapeJsString(hexId)}')">
      Open Details
    </button>
  `;
}
          ...
          <span>
            ${
              crumb.clickable && !isLast
                ? `<button class="codex-link" type="button" onclick="${crumb.onclick}">${escapeHtml(crumb.label)}</button>`
...
function renderCodexListControls(config) {
  ...
  return `
    <div class="codex-list-controls">
      ${filters.map(filter => `
        <div class="codex-filter-row">
          <select id="${escapeHtml(filter.fieldId || `${filter.id}-field`)}">
            ${renderCodexSelectOptions(
              filter.fieldOptions || [
                { value: filter.fieldValue || filter.id, label: filter.label }
              ],
      ...
      <div class="codex-sort-row">
        <label for="${escapeHtml(config.sortId)}">Sort</label>
...
function renderCodexHexPage(hexId) {
  ...
  setCodexContent(`
    ...
    <p><strong>Region:</strong> ${
      region
        ? `<button class="codex-link" type="button" onclick="openCodexPage('region', '${escapeJsString(region.Region_ID)}')">${escapeHtml(region.Region_Name)}</button>`
...
function renderCodexRegionPage(regionId) {
  ...
  setCodexContent(`
    ...
    <p>${renderMultilineText(terrainSummary || "No terrain data recorded.")}</p>
  ...
  `, [
    { label: "Codex", clickable: true, onclick: "resetCodexToIndex()" },
    { label: "Regions", clickable: true, onclick: "openCodexPage('regions')" },
    { label: regionName }
  ]);
}
...
function renderCodexPoiPage(poiId) {
  ...
  setCodexContent(`
    ...
    ${
      hexId
        ? `<p><strong>Hex:</strong> <button class="codex-link" type="button" onclick="openCodexPage('hex', '${escapeJsString(hexId)}')">${escapeHtml(hexId)}</button></p>`
        : ""
    }
  ...
  `, [
    { label: "Codex", clickable: true, onclick: "resetCodexToIndex()" },
    { label: "Points of Interest", clickable: true, onclick: "openCodexPage('pois')" },
    { label: poiName }
  ]);
}
...
function getNpcFilterOptions(field) {
  ...
  return [
    { value: "all", label: "All" },
    ...getUniqueValues(npcs, npc => getNpcFilterValue(npc, field))
...
function updateNpcFilterValueOptions(fieldSelectId, valueSelectId) {
  ...
  valueSelect.value = "all";
}

function getPoiPopulationNumber(poi) {
  return Number(String(poi?.Population || "").replace(/[^\d]/g, "")) || 0;
}
...
function renderPoiListIntoContainer() {
  ...
  pois = applyFilters(pois, [
    { field: "POI_Type", value: typeFilter },
    { field: "Notoriety Tier", value: notorietyFilter }
  ]);

  let compareFn = (a, b) => compareText(a.Name, b.Name);
  ...
  if (sortMode === "population") {
    compareFn = (a, b) => {
      const primary = getPoiPopulationNumber(a) - getPoiPopulationNumber(b);
      return primary !== 0 ? primary : compareText(a.Name, b.Name);
    };
  }
...
function renderCodexPoisIndex() {
  ...
  const poiTypes = getUniqueValues(pois, poi => poi.POI_Type);
  const poiNotorietyTiers = getUniqueValues(pois, poi => poi["Notoriety Tier"]);
      ...
      filters: [
        {
          ...
          options: [
            { value: "all", label: "All" },
            ...poiTypes.map(type => ({ value: type, label: type }))
          ]
        },
        {
          ...
          options: [
            { value: "all", label: "All" },
            ...poiNotorietyTiers.map(tier => ({ value: tier, label: tier }))
          ]
        }
      ],
  ...
  document.getElementById("codex-poi-type-filter")
    .addEventListener("change", renderPoiListIntoContainer);

  document.getElementById("codex-poi-notoriety-filter")
    .addEventListener("change", renderPoiListIntoContainer);

  document.getElementById("codex-poi-sort")
    .addEventListener("change", renderPoiListIntoContainer);

  document.getElementById("codex-poi-direction")
...
function renderCodexNpcsIndex() {
  ...
  document.getElementById("codex-npc-filter-1-field")
  ...
  document.getElementById("codex-npc-filter-2-field")
    .addEventListener("change", function () {
      updateNpcFilterValueOptions(
  ...
  document.getElementById("codex-npc-filter-1-value")
    .addEventListener("change", renderNpcListIntoContainer);

  document.getElementById("codex-npc-filter-2-value")
    .addEventListener("change", renderNpcListIntoContainer);

  document.getElementById("codex-npc-sort")
    .addEventListener("change", renderNpcListIntoContainer);

  document.getElementById("codex-npc-direction")
    .addEventListener("click", function () {
      const current = this.dataset.direction || "asc";
      const next = current === "asc" ? "desc" : "asc";
...
function renderCodexSearchPage() {
  ...
  content.innerHTML = `
    <input
      ...
      type="search"
      placeholder="Search regions, hexes, POIs, NPCs..."
      autocomplete="off"
    />
...
function renderCodexLinkedList(
  ...
  return `
    <div class="codex-linked-list">
      ${rows.map(row => {
        const id = row?.[idField];
        const resolvedType = getType ? getType(row) : type;
        ...
        return `
          <button
            class="codex-linked-list-item"
            type="button"
            onclick="openCodexPage('${escapeJsString(resolvedType)}', '${escapeJsString(id)}')"
          >
            <span class="codex-linked-list-title">${escapeHtml(title)}</span>
            ${metaLines.map(line => `
              <span class="codex-linked-list-meta">${escapeHtml(line)}</span>
            `).join("")}
            <span class="codex-linked-list-arrow">›</span>
          </button>
        `;
      }).join("")}
    </div>
...
function buildMobilePopupHtml(hexId) {
  ...
  return `
    <div class="mobile-hex-popup">
      <strong>${escapeHtml(hexId)}</strong>
      <div>${escapeHtml(data?.Terrain || "Unknown")}</div>
  ...
  for (let yyy = 300; yyy < 350; yyy++) {
    ...
    hex.on("click", function (e) {
      ...
      if (isTouchDevice) {
        ...
        const panelWidth = document.getElementById("app-panel").offsetWidth;
